...
|----------|----------|----------|
| **Базовый URL API** |  | Основной эндпоинт |
| **Аутентификация** | OAuth 2.0 | [Подробнее в разделе "Аутентификация"]() |
| **Лимиты** |  | Ограничение на количество запросов в минуту |
| **Частота перезаписи данных** |  | Частота обновления тестовой БД |
...
| Параметр | Значение | Описание |
|----------|----------|----------|
| **Базовый URL API** | | Продовский эндпоинт |
| **Аутентификация** | OAuth 2.0 | [Подробнее в разделе "Аутентификация"]() |
| **Лимиты** | | Ограничение на количество запросов в минуту |
| **SLA** | | Обслуживание |
...
Определяет, какие особые обязанности есть у сотрудника в компании. Например, кто является руководителем или главным бухгалтером. В каждом подразделении может быть только один человек с такой ролью. Сотрудник может работать в одной компании, а роль выполнять в другой.
| Поле | Обязательность | Тип | Формат | Пример | Описание |
|------|----------------|-----|---------|---------|-----------|
